...
const senderEmail = "enderlifesmpog@gmail.com";
...
async function getBrevoApiKey(): Promise<string> {
  const { data, error } = await supabase.rpc("get_brevo_api_key");

  if (error || !data) {
    throw new Error("Brevo API key not found in vault");
  }
  return data as string;
}

async function sendBrevoEmail(toEmail: string, code: string, brevoApiKey: string): Promise<void> {
  const url = "https://api.brevo.com/v3/smtp/email";
  const payload = {
    sender: { email: senderEmail, name: "LightXZ" },
    to: [{ email: toEmail }],
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    try {
      brevoApiKey = await getBrevoApiKey();
    } catch (vaultErr) {
      console.error("Vault access failed:", vaultErr instanceof Error ? vaultErr.message : String(vaultErr));
      return json(500, { ok: false, error: "Email service not configured. Please contact support." });
    }

    // Send email via Brevo
    try {
      await sendBrevoEmail(normalizedEmail, code, brevoApiKey);
    } catch (emailErr) {
